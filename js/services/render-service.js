...
function showEmptyState(container, message) {
    ...
    const sanitizedMessage = typeof message === 'string' ? escapeHtml(message) : 'Data tidak tersedia';
...
function renderClauseCard(clauseData, handleClauseClick) {
    ...
    const introductionText = clauseData.clause.introduction 
        ? escapeHtml(clauseData.clause.introduction.substring(0, 100) + '...')
        : 'Deskripsi tidak tersedia';
    
    const subClauseCount = Array.isArray(clauseData.subClauses) ? clauseData.subClauses.length : 0;
    
    clauseCard.innerHTML = `
        <div class="clause-card-inner" data-clause="${escapeHtml(clauseData.clause.number)}">
            <div class="clause-header d-flex justify-content-between align-items-center">
                <span>Klausul ${escapeHtml(clauseData.clause.number)}: ${escapeHtml(clauseData.clause.title)}</span>
...
function renderSubclauseItem(subclause, clauseNumber, showSubclauseDetail) {
    ...
    const descriptionText = subclause.description 
        ? escapeHtml(subclause.description.substring(0, 80) + '...')
        : 'Deskripsi tidak tersedia';
    
    subclauseItem.innerHTML = `
        <div class="d-flex justify-content-between align-items-center">
            <strong>${escapeHtml(subclause.number)} ${escapeHtml(subclause.title)}</strong>
...
function renderDocuments(documentsList, subclause) {
    ...
    subclause.documents.forEach((doc, index) => {
        ...
        docItem.innerHTML = `
            ...
            <p class="mb-0">${escapeHtml(doc.description || 'Tidak ada deskripsi')}</p>
        `;
...
function renderGuidance(guidanceContent, clauseData) {
    ...
    clauseData.implementationGuidance.steps.forEach((step, index) => {
        ...
        const activitiesHTML = Array.isArray(step.activities) 
            ? step.activities.map(activity => `<li class="mb-1">• ${escapeHtml(activity || '')}</li>`).join('') 
            : '';
        
        stepItem.innerHTML = `
            <h6 class="mb-2">${escapeHtml(step.title || 'Step')}</h6>
            <p class="small text-muted mb-2">${escapeHtml(step.description || '')}</p>
