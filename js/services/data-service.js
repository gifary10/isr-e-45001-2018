...
async function loadClauseData(clauseNumber) {
    try {
        ...
        if (isNaN(clauseNumber)) {
            throw new Error('Invalid clause number: must be a number');
        }
        if (clauseNumber < 1 || clauseNumber > 10) {
            throw new Error(`Invalid clause number: ${clauseNumber}. Must be between 1-10`);
        }
        ...
        // Fetch fresh data
        const response = await fetch(`./clauses/clause${clauseNumber}.json`);
        if (!response.ok) {
